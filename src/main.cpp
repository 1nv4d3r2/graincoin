...
#if defined(NDEBUG)
# error "Graincoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1f85926b495f6e3746546fa5ebfef4c94727aa0d045ffb587e5177a182f2cb71");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Graincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Graincoin Signed Message:\n";
...
{
    // Graincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 540000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 540000); // Graincoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Graincoin: 3 days
static const int64 nTargetSpacing = 2.5 * 60; // Graincoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Graincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Graincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc80157bdbdeeba3a5bde058a025ca0d70e267a429bdc67729f1f4b0e3f655759");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BR Brasil 09/Nov/2019 Lula Free, Brasil Visionary.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042f82ac42185ad99e5e3a9c4f5442113dc537562154c0659b3a26dad9c038b8e3da08c13a01994d3b06b69f55bc990a706ec5faf0c9d991910301f0c601bf6470") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1574734246;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089252194;

        if (fTestNet)
        {
            block.nTime    = 1574734204;
            block.nNonce   = 387388043;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb914098d948abf47da227c8082edd403f47fe92f42fe963cef5296b521eb6acb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xba, 0xdc }; // Graincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GraincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GraincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GraincoinMiner(CWallet *pwallet)
{
    printf("GraincoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GraincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GraincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GraincoinMiner, pwallet));
}
